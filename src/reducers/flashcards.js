...
const flashcards = (flashcards = SAMPLE_FLASHCARDS, action) => {
    ...
    switch (action.type) {
        ...
        case 'DELETE_FLASHCARD':
            ...
            const cards = newFlashcards.filter(card => Number(card.id) !== Number(action.payload));
            // cards.map(card => console.log(card.id));
            return cards;
        case 'CHANGE_TERM':
            const cards1 = newFlashcards.filter(card => Number(card.id) === Number(action.id));
            const newCard1 = cards1.at(0);
            console.log(newCard1.term);
            newCard1.term = action.payload;
            console.log(newCard1.term);
            const cards2 = newFlashcards.map(card => (Number(card.id) === Number(action.id)) ? (newCard1) : (card));
            cards2.map(card => console.log(card.term));
            return cards2;
        case 'CHANGE_DEFINITION':
            const cards3 = newFlashcards.filter(card => Number(card.id) === Number(action.id));
            const newCard2 = cards3.at(0);
            console.log(newCard2.definition);
            newCard2.definition = action.payload;
            console.log(newCard2.definition);
            const cards4 = newFlashcards.map(card => (Number(card.id) === Number(action.id)) ? (newCard2) : (card));
            cards4.map(card => console.log(card.definition));
            return cards4;
        case 'CHANGE_DESCRIPTION':
            const cards5 = newFlashcards.filter(card => Number(card.id) === Number(action.id));
            const newCard3 = cards5.at(0);
            console.log(newCard3.description);
            newCard3.description = action.payload;
            console.log(newCard3.description);
            const cards6 = newFlashcards.map(card => (Number(card.id) === Number(action.id)) ? (newCard3) : (card));
            cards6.map(card => console.log(card.description));
            return cards6;
        case 'CHANGE_IMAGEURL':
            const cards7 = newFlashcards.filter(card => Number(card.id) === Number(action.id));
            const newCard4 = cards7.at(0);
            console.log(newCard4.imageURL);
            newCard4.imageURL = action.payload;
            console.log(newCard4.imageURL);
            const cards8 = newFlashcards.map(card => (Number(card.id) === Number(action.id)) ? (newCard4) : (card));
...
const SAMPLE_FLASHCARDS = [
    {
        ...
        term: 'vin',
        definition: 'wine',
        description: 'Coffret Découverte 3 Bouteilles - Saint Emilion Grand Cru & Castillon Côtes de Bordeaux',
    ...
    },
    {
        ...
        term: 'fromage',
        definition: 'cheese',
        description: 'DLC minimum garantie: 7 jours - Le Chaource est un fromage naturel fabriqué à partir de lait de vache entier selon des méthodes traditionnelles. CHAOURCE BOITE 250G Pour votre santé, mangez au moins cinq fruits et légumes par jour. FROMAGERIE LINCET 15 RUE DE LA QUENNEVELLE - 89 100 SALIGNY',
